...
class SignupForm(forms.ModelForm):
    ...
    class Meta:
        model = User
        fields = ('accountType','experience','username', 'email', 'password')
...
class EditProfileForm(forms.ModelForm):
      ...
      profile_info = forms.CharField(widget=forms.Textarea(attrs={'class':'input is-medium'}), max_length=160, required=False)
      experience = forms.CharField(widget=forms.TextInput(attrs={'class':'input is-medium'}), max_length=25, required=False)
      phonenumber = forms.CharField(widget=forms.TextInput(attrs={'class':'input is-medium'}), max_length=25, required=False)
      camera_details = forms.CharField(widget=forms.TextInput(attrs={'class':'input is-medium'}), max_length=25, required=False)
      email = forms.CharField(widget=forms.TextInput(attrs={'class':'input is-medium'}), max_length=25, required=False)
      rating = forms.IntegerField(widget=forms.TextInput(attrs={'class':'input is-medium'}),  required=False)
      
      class Meta:
          model=Profile
          fields = ('picture', 'first_name', 'last_name', 'location', 'url', 'profile_info','experience','phonenumber','camera_details','email','rating')